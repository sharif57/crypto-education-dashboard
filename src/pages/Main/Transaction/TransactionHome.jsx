...
export default function TransactionHome() {
  ...
  const handleActionClick = (user) => {
    setSelectedUser({
      ...user,
      userId: `#446${user.sl}78`, // Dynamic userId based on sl for uniqueness
    });
    setIsModalOpen(true);
  };
  ...
  return (
    <div className="min-h-screen bg-[#373737] rounded-lg">
      ...
      <div className="flex items-center justify-between mb-6 px-6 py-4">
        <h1 className="text-white text-2xl font-medium">Earnings</h1>
      ...
      <div className="w-full overflow-hidden">
        {/* Table Header */}
        <div className="bg-[#62C1BF] px-6 py-4 grid grid-cols-7 gap-4 text-white font-medium text-sm">
          <div>#SL</div>
          <div>User Name</div>
          <div>Email</div>
          <div>Date</div>
        ...
        <div className="divide-y divide-gray-600">
          {users.map((user) => (
            <div
              key={user.id}
              className="px-6 py-4 grid grid-cols-7 gap-4 text-gray-300 text-sm hover:bg-gray-600 transition-colors"
            >
              <div>#{user.sl}</div>
              <div>{user.userName}</div>
              <div>{user.email}</div>
              <div>{user.date}</div>
              <div>{user.transactionAmount}</div>
