...
export default function RelatedVideo() {
  ...
  try {
    // setLoading(true);
    const res = await deleteCategory(videoId).unwrap();

    if (res?.status === "success") {
      toast.success("Category deleted successfully!" || res?.message);
    }
  } catch (error) {
    toast.error("Failed to delete category" || error?.data?.message);
  } finally {
    setLoading(false);
  }
};
