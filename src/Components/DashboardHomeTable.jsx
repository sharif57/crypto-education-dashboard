...
import { useState, useMemo } from "react";
import { Search, Info, X, Download, Plus, EyeOff, Eye } from "lucide-react";
import { useAddUserMutation, useUserListQuery, useUpdateUserSubscriptionMutation } from "../redux/features/useSlice";
import Loading from "./Loading";
...
export default function DashboardHomeTable() {
 ...
 const [formData, setFormData] = useState({
  ...
  // Open subscription update modal
  const openUpdateSubscriptionModal = (user) => {
  ...
  // Close subscription update modal
  ...
  // Handle subscription update submission
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-700 rounded-lg p-6 w-full max-w-md relative">
            ...
            <div className="flex gap-3">
              <button
                onClick={handleDownload}
                className="flex-1 bg-teal-500 hover:bg-teal-600 text-white py-3 rounded-md font-medium transition-colors flex items-center justify-center gap-2"
              >
                <Download className="w-4 h-4" />
                Download
              </button>
              <button
                onClick={() => {
                  closeDetailsModal();
                  openUpdateSubscriptionModal(selectedUser);
                }}
                className="flex-1 bg-blue-500 hover:bg-blue-600 text-white py-3 rounded-md font-medium transition-colors"
              >
                Update Subscription
              </button>
            </div>
...
{isUpdateSubscriptionModalOpen && selectedUser && (
  <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div className="bg-gray-700 rounded-lg p-6 w-full max-w-md relative">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-white text-xl font-medium">Update Subscription</h2>
        <button
          onClick={closeUpdateSubscriptionModal}
          className="w-8 h-8 bg-red-500 rounded-full flex items-center justify-center text-white hover:bg-red-600"
        >
          <X className="w-5 h-5" />
        </button>
      ...
      <form onSubmit={handleUpdateSubscription} className="space-y-5">
        <div>
          ...
          <div className="bg-gray-800 text-white px-4 py-3 rounded-md">
            {selectedUser.fullName}
          </div>
        </div>

        <div>
          <label className="block text-gray-300 text-sm mb-2">Subscription Type</label>
          <select
            name="subscription"
            value={subscriptionData.subscription}
            onChange={handleSubscriptionInputChange}
            required
            className="w-full bg-gray-800 text-white px-4 py-3 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 cursor-pointer"
          >
            <option value="free">Free</option>
            <option value="pro">Pro</option>
            <option value="elite">Elite</option>
        ...
        <div>
          <label className="block text-gray-300 text-sm mb-2">Billing Cycle</label>
          <select
            name="billing_cycle"
